...
```haskell title="GHCi session (constructors are functions)" showLineNumbers=false frame="terminal"
...
```haskell title="Student.hs" startLineNumber=2
x, y :: Student
...
```haskell title="GHCi session (students and record syntax)" showLineNumbers=false frame="terminal"
ghci> :l Student.hs
...
ghci> name x
"Alice"
...
In Haskell and Python (the expression in parentheses is a lambda
expression):

<Tabs syncKey="lang">
  <TabItem label="Haskell" icon="seti:haskell">
    ```haskell title="GHCi session (lambda expressions)" showLineNumbers=false frame="terminal"
    ghci> (\x -> x + 4) 3
    7
    ```
  </TabItem>
  <TabItem label="Python" icon="seti:python">
    ```python title="Python interactive session (lambda expressions)" showLineNumbers=false frame="terminal"
    >>> (lambda x: x + 4)(3)
    7
    ```
  </TabItem>
...
```haskell title="GHCi session (explicit type applications)" showLineNumbers=false frame="terminal" {1}
...
```haskell title="GHCi session (kinds)" showLineNumbers=false frame="terminal" ":k"
...
```haskell title="GHCi session (higher kinds)" showLineNumbers=false frame="terminal" 
...
beginning of [Chapter 2.2 (Polymorphism)](/it5100a-notes/notes/types/02-polymorphism).

## Other Polymorphisms

At the start of [Chapter 2.2 (Polymorphism)](/it5100a-notes/notes/types/02-polymorphism) we introduced three questions, two of
...
<Tabs>
  <TabItem label="ADTs as type aliases">
    ```python
  ...
  <TabItem label="ADTs as superclasses">
    ```python
...
```python title="Expr.py" startLineNumber=5
@dataclass
...
```python title="Expr.py" startLineNumber=17
@dataclass
...
```python title="Expr.py" startLineNumber=23
@dataclass
...
``` python title="Expr.py" startLineNumber=30
