...
- In the function header, we have a specification for `f` to receive an
...
automatically), so our type annotation for it is not necessary.
- Outside the function body we have other type annotations, documenting what the
