...
```haskell showLineNumbers=false frame="terminal" title="Python interactive session session (+)"
>>> type(1 + 1)
class <'int'>
>>> type(1 + 1.0)
class <'float'>
...
```haskell title="HAdd.hs" startLineNumber=3
...
```haskell title="HAdd.hs" ins={1,3} del={2} showLineNumbers=false
