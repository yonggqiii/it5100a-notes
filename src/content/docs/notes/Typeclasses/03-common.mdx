---
title: Commonly Used Typeclasses
---
...
re-using the `Fraction` algebraic data type defined in [Chapter 2.3 (Algebraic Data Types)](/it5100a-notes/notes/types/03-adt/#algebraic-data-types):
...
``` haskell title="Fraction.hs" startLineNumber=2
...
```haskell title="GHCi session (Eq)" frame="terminal" showLineNumbers=false
ghci> :i Eq
type Eq :: * -> Constraint
class Eq a where
  (==) :: a -> a -> Bool
...
``` haskell title="Fraction.hs" startLineNumber=2 del={4-6}
instance Eq Fraction where
  (==) :: Fraction -> Fraction -> Bool
  F a b == F c d = a == c && b == d
...
```haskell title="GHCi session (equality of fractions)" frame="terminal" showLineNumbers=false
...
```haskell title="Tree.hs" del={4} ins={5} showLineNumbers=false
data Tree a = Node (Tree a) a (Tree a)
            | Empty
...
```haskell title="GHCi session (derived instances)" frame="terminal" showLineNumbers=false
