---
title: The Existential Typeclass Pattern
---
...
```
Indeed, **polymorphic functions simulate functions over existential types**. Let us show more examples of this being the case. For example, the `area` typeclass method is a function over _something that implements `HasArea`_, and returns a `Double`. Therefore, it should have the following function signature:
```haskell showLineNumbers=false
