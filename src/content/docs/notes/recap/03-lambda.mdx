...
:::theorem{#def:fv title="Free Variable"}
A variable $x$ in a $\lambda$ term $e$ is 
...
:::theorem{#def:substitution title="Substitution"}
$e_1[x := e_2]$ is the substitution of all _free_ occurrences of $x$ in $e_1$ with $e_2$, changing the names of bound variables to avoid name clashes. Substitution is defined by the following rules:
1. $x[x := e] \equiv e$
...
:::theorem{#eg:subtitution title="Substitution"}
...
The last rule of :autoref{#def:substitution} where variables are renamed to avoid name clashes introduces a form of equivalence known as $\alpha$ congruence. It captures the idea that renaming parameters in functions does not change its meaning. For example, the two functions below are, in operation, identical:
...
```python frame="terminal" title="Python interactive session (lambda expressions)" showLineNumbers=false
...
```python frame="terminal" title="Python interactive session (types)" showLineNumbers=false
>>> type(1)
<class 'int'>
>>> type(1.0)
<class 'float'>
```
We will describe more on types in [Chapter 2 (Types)](/it5100a-notes/notes/types/01-type-systems). But for now, know that we can also assign types to terms in the $\lambda$ calculus, giving us new forms of $\lambda$ calculi. The simplest type system we can add to the $\lambda$ calculus is, well, simple types, forming the _simply-typed_ $\lambda$ _calculus_.
...
```python frame="terminal" title="Python interactive session (types and lambda expressions)" showLineNumbers=false
