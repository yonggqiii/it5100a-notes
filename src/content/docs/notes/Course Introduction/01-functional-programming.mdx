...
```
:::tip
Impurity is inevitable in a useful program. Purely functional programming languages use a variety of techniques to allow us to write impure functions. We discuss this in [Chapter 5 (Monads)](/notes/monads/01-more-on-monads).
...
x: int = 'abc'
```
No type errors will be thrown by the Python interpreter. However, you can enforce adherence to type annotations with external programs. We discuss this in [Chapter 2.1 (Type Systems)](/notes/types/01-type-systems).
:::
...
``` python {"1. The add function is curried.":23-25} {"2. add(2) receives x and returns x + 2.":28} title="map.py" collapse={2-22} collapseStyle="collapsible-start"
# Data types
@dataclass(frozen=True)
class Tree:
    def map(self, f):
