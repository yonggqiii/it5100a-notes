...
Object-Oriented Programming (OOP) has four principles as you might recall: **Abstraction**, **Inheritance**, **Encapsulation** and **Polymorphism**.[^1] Functional Programming, on the other hand, is centered around the following principles, which really are just principles of mathematical functions and the $\lambda$ *calculus*:[^4] 
...
:::note
Indeed, having all data be immutable can incur a time/space performance overhead. For example, if we use a dynamically sized array for list-like structures, insertion, updates, deletions become in $O(n)$ time/space. There are better **purely functional data structures** we can use, and sometimes the performance overhead is worth the benefits that immutability comes with.
:::
...
```python title="impure-converted.py"
def f(ls):
    x = ls
    addend = x[-1] + 1
    x = x + [addend]
...
```
:::tip
Impurity is inevitable in a useful program. Purely functional programming languages use a variety of techniques to allow us to write impure functions. We discuss this in [Chapter 5 (Monads)](/it5100a-notes/notes/monads/01-more-on-monads).

In general, it can be a good idea to separate impure functions from pure functions, and to keep as many functions (as reasonably possible) pure. We call this the principle of "separating Church and state" (referring to Alonzo Church).
:::
...
reassignments to `row` and `num` (the loop variables) in lines 3 and 4,
and the mutation of the `total` variable in the loop body (line 5). In
...
``` python title="recursive-sum-2d.py"
def row_sum(row):
...
recursive. Take the example of obtaining the pre-order traversal of a binary tree.
Binary trees are recursive data structures, if formulated the following
way:

:::theorem{#def:binary-tree title="Binary Tree"}
...
obtaining the **pre-order traversal** of a tree can be like so:

:::theorem{#def:preorder title="Pre-order Traversal"}
1.  The pre-order traversal of an empty tree is the empty list
2.  The pre-order traversal of a node is the node's value, together with the
...
``` python title="preorder.py" collapse={4-16} collapseStyle="collapsible-start"
...
```
:::danger
Recursion is generally frowned upon in imperative languages like Python, especially when the stack size is limited. This is because each recursive call can push one frame onto the call stack, and without [tail-call optimization](https://en.wikipedia.org/wiki/Tail_call), stack overflow can occur for deeply recursive computations.
:::
...
to it, then the last line (line 3) containing `x + 5` will definitely not cause a `TypeError`, because we know that adding any number to an integer will always work.

Contrast the above with the following example.


``` python title="unsafe-div.py"
...
```python title="safe-div.py" startLineNumber=5
x: int = int(input())
y: int = int(input())
z: Maybe[int]
match safe_div(x, y):
...
:::caution
Type annotations in Python are merely **suggestions**. If you write the following:
```python showLineNumbers=false
x: int = 'abc'
```
No type errors will be thrown by the Python interpreter. However, you can enforce adherence to type annotations with external programs. We discuss this in [Chapter 2.1 (Type Systems)](/it5100a-notes/notes/types/01-type-systems).
:::
...
support **code re-use**. Higher-order functions can receive functions as arguments and/or return functions as output. 
...
``` python {"1. The add function is curried.":23-25} {"2. add(2) receives x and returns x + 2.":28-29} title="map.py" collapse={2-22} collapseStyle="collapsible-start"
# Data types
@dataclass(frozen=True)
class Tree:
    def map(self, f):
...
```haskell showLineNumbers=false frame="terminal" title="GHCi session"
ghci> x = [1, 2, 3]
ghci> map (+2) x
[3,4,5]
```
...
[^4]: If you have not, you may want to read [a recap on the $\lambda$ calculus](/it5100a-notes/notes/recap/03-lambda) before continuing.

[^2]: If you have not, you may want to read
    [a recap on recursion](/it5100a-notes/notes/recap/01-recursion) before continuing.
...
[^7]: If you have not, you may want to read a 
    [a recap on first-class functions](/it5100a-notes/notes/recap/02-first-class-functions) before continuing.
