...
```haskell title="Example.hs" startLineNumber={15}
...
```haskell frame="terminal" title="GHCi session (parsing users)" showLineNumbers=false
...
```haskell del={1,5,6,10,14,15} ins={2,7,8,11,16,17}
parseEmail :: String -> Maybe Email
...
```haskell title="GHCi session (parsing users)" frame="terminal" showLineNumbers=false
ghci> parseUser "Foo" "yong@qi.com" "1000"
...
```haskell title="Validation.hs" startLineNumber={7}
...
```haskell title="Example.hs" del={1,5,6,10,14,15} ins={2,7,8,11,16,17}
parseEmail :: String -> Either String Email
...
```haskell title="Example.hs" del={4} ins={5}
parseUser :: String -- name
             -> String -- email
             -> String -- salary
             -> Either String User -- user
