...
## Question 2
Without the help of GHCI, describe the types of `eqLast`, `isPalindrome`, `burgerPrice` and `(@:)` which we defined in [Exercise 1 (Haskell Basics)](/it5100a-notes/exercises/exercises/01-intro).
...
```haskell title="GHCi session (dropConsecutiveDuplicates example run)" frame="terminal" showLineNumbers=false
...
Example runs follow:
```haskell title="GHCi session (zippers example run)" frame="terminal" showLineNumbers=false
...
such sets is the Binary Search Tree (BST) abstract data type (ADT). For those
already know what BSTs are, skip ahead to the question. Otherwise, we summarize
what BSTs are and how operations on them are defined.


### Definition of BST
First of all, let us define what a BST is.

:::theorem{#def:tree title="Tree"}
A Tree is an abstract data structure representing a hierarchical tree structure with a set of connected nodes. Each node can be connected to many children but must be connected to exactly one parent, except for the root node which has no parent. Trees have no cycles or loops.
:::

:::theorem{#def:bin-tree title="Binary Tree"}
A Binary Tree is a tree where each node has up to two children.
:::

Here is an example of a binary tree of integers.
...
The root is the node on top with value `4` and has two children. Some of the other nodes have 0, 1 or two children. There are no cycles in this tree.

Notice that this definition allows us to define binary trees recursively, just like how lists are defined. A binary tree is either:
1. The empty tree
2. A node with a left child (also a binary tree), a value at the node, and a right child (also a binary tree).

:::theorem{#def:bintree title="Binary Search Tree"}
A Binary Search Tree (BST) is a binary tree with the following invariants:
1. Values in the BST are all unique
2. Suppose we have a node $n$ with left child $l$ and right child $r$. The value at $n$ is larger than all nodes in $l$ and smaller than all nodes in $r$.
:::

Here is an example BST of integers. Notice that the value of any node is larger than all the values in its left child (if it exists) and smaller than all the values in the right child (if it exists). The values are also all unique.
...
### Operations on BSTs
There are several operations we might consider. 


#### Search
The first is checking whether an element is a member of a BST.
The algorithm for doing so is quite simple. Given an element $$x$$ and a tree $$t$$:
1. If $$t$$ is empty, then return False.
2. Otherwise, $$t$$ is a node with left child $$l$$, right child $$r$$ and value $$v$$.
    1. If $$x = v$$, then return True.
    2. If $$x < v$$, $$x$$ must be in the left child $$l$$, so recursively search for $$x$$ in $$l$$.
    3. Otherwise, $$x > v$$ and thus $$x$$ must be in the right child $$r$$, so recursively search for $$x$$ in $$r$$.

#### Insertion
The next is inserting an element to a BST. Doing so is very similar to search.
Given an element to insert $$x$$ and tree $$t$$:
1. If $$t$$ is empty, then return a node $$n$$ with empty children and value $$x$$.
2. Otherwise, $$t$$ is a node with left child $$l$$, right child $$r$$ and value $$v$$.
    1. If $$x = v$$, then return $$t$$ itself unmodified since values in a BST are unique.
    2. If $$x < v$$, $$x$$ must be in the left child $$l$$, so recursively insert $$x$$ into $$l$$, returning $$l'$$. Return a new tree $$t'$$ with the new left child $$l'$$, the same value $$v$$ and the same right child $$r$$.
    3. Otherwise, $$x > v$$ and thus $$x$$ must be in the right child $$r$$, so recursively insert $$x$$ into $$r$$ returning $$r'$$. Return a new tree $$t'$$ with the same left child $$l$$, same value $$v$$ and the new right child $$r'$$.

#### Values in List 
The last operation we can do is to obtain a sorted list of all the elements in a BST. By design, the BST keeps track of the ordering of the elements within it. This makes obtaining a sorted list of all the elements within it incredibly simple. The way we do so is via an in-order traversal of the BST $$t$$, defined as follows:
1. If $$t$$ is empty, return the empty list
2. Otherwise, $$t$$ is a node with left child $$l$$, value $$v$$ and right child $$r$$. The in-order traversal of $$t$$ is the in-order traversal of $$l$$, concatenated with the node $$v$$, then concatenated with the in-order traversal of $$r$$.
...
signature of those functions. For example: the type of `setToList` might have `Ord a => SortedSet a -> [a]`.
Example runs follow:

```haskell title="GHCi session (sorted sets example run)" frame="terminal" showLineNumbers=false
...
```haskell title="GHCi session (expressions example run)" frame="terminal" showLineNumbers=false
ghci> n = LitNumExpr
ghci> b = LitBoolExpr
ghci> a = AndExpr
ghci> o = OrExpr
...
```python title="Python interactive session (accounts example run)" frame="terminal" showLineNumbers=false
...
:::note
The type of a tuple with two elements of types `A` and
`B` is `tuple[A, B]`.
:::
Example runs follow:

```python title="Python interactive session (simple operations example run)" frame="terminal" showLineNumbers=false
...
```python title="Python interactive session (advanced features example run)" frame="terminal" showLineNumbers=false
>>> x = NormalAccount('abc', 1000, 0.01)
>>> y = MinimalAccount('bcd', 2000, 0.02)
...
```python title="Python interactive session (example database)" frame="terminal" showLineNumbers=false
...
```python title="Python interactive session (process_one example run)" frame="terminal" showLineNumbers=false
# data
...
```python title="Python interactive session (process_all example run)" frame="terminal" showLineNumbers=false
# data
>>> alice = NormalAccount('alice', 1000, 0.1)
>>> bob = MinimalAccount('bob', 999, 0.1)
>>> mp = {'alice': alice, 'bob': bob}
...
```python title="Python interactive session (process example run)" frame="terminal" showLineNumbers=false
# data
>>> alice = NormalAccount('alice', 1000, 0.1)
>>> bob = MinimalAccount('bob', 999, 0.1)
>>> mp = {'alice': alice, 'bob': bob}
...
```python title="Python interactive session (gather_primes)" frame="terminal" showLineNumbers=false
...
```python title="Python interactive session (gather_primes example run)" frame="terminal" showLineNumbers=false
>>> gather_primes(2, [])
(True, [2])
>>> gather_primes(3, [2])
(True, [2, 3])
...
```python title="Python interactive session (process with gather_primes example run)" frame="terminal" showLineNumbers=false
...
:::note
The type of a function that receives parameters `A`, `B` and `C`
