...
#### Lists
```python frame="terminal" showLineNumbers=false title="Python interactive session"
# lists
...
Recall Question 6 in [Chapter 1 (Exercises)](/it5100a-notes/exercises/exercises/01-intro/) where we defined a function `sumDigits` in Haskell. Now write a function `sum_digits(n)` that does the same, i.e. sums the digits of a nonnegative integer $n$, in Python. Example runs follow:
...
```haskell frame="terminal" showLineNumbers=false title="GHCi session"
ghci> sumDigits 1234
10
...
```
:::tip
Use `do`-notation in your Haskell implementation!
:::
...
```
:::note
The `split` function in Haskell was defined in the hands-on section in [Chapter 4.4 (Railway Pattern#Validation)](/it5100a-notes/notes/railway/04-validation/).
:::
...
:::tip
With the `-XApplicativeDo` extension, you can actually use `do` notation on `Functor`s and `Applicative`s. Give it a try by defining `choose` using `do`-notation! For more information on the conditions for when you can use `Applicative` `do`-notation, see the [GHC Users Guide](https://ghc.gitlab.haskell.org/ghc/doc/users_guide/exts/applicative_do.html).
:::

:::note
`Validation` is not included in Haskell's Prelude. You can use the `Validation` datatype definition and its supporting typeclass instances as defined in the hands-on portion of [Chapter 4.4 (Railway Pattern#Validation)](/it5100a-notes/notes/railway/04-validation/).
:::
