...
23. `30`. The easiest way to see this is by converting this to
    ...
    ```python frame="terminal" title="Python interactive session" showLineNumbers=false
    >>> sum([x[0] for x in 
...
``` haskell title="IsPalindrome.hs"
isPalindrome w = w == reverse w
```

### Question 4

```haskell title="TaxiFare.hs"
...
```haskell title="BurgerPrice.hs" startLineNumber={8}
burgerPrice burger 
  | null burger = 0
  | otherwise = 
...
`map ingredientPrice`. We have shown that 
...
```haskell frame="terminal" title="GHCi session" showLineNumbers=false
ghci> show 123
"123"
```
